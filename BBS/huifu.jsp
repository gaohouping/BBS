...
function CheckSubmit()
 {
  ...
  if( document.form.context.value.length >=100 )
     { alert("留言内容不能大于50字!"); document.form.context.focus(); return false; } 		 
  return true;                                                                                          
 } 
</SCRIPT>
<%
  ...
  String subject=(String)session.getAttribute("id");	
  ...
  String s_advice="";
  	  String id="";
  rs=stmt.executeQuery("select count(*) from notess  ");
  rs.next();
...
<%if(i_totalnum!=0){%>
        <tr> 
          ...
          <td colspan="3"><div align="right">
            第<%=i_currentpage%>页&nbsp;
<%          if(i_currentpage>1){%>
              <a href="huifu.jsp?op=firstpage">首页</a>
<%          }else{%>首页<%}%>&nbsp;
<%          if(i_currentpage>1){%>
              <a href="huifu.jsp?op=previous&currentpage=<%=i_currentpage%>">上一页</a> 
<%          }else{%>上一页<%}%>&nbsp;
<%          if(i_currentpage!=i_totalpage){%>
              <a href="huifu.jsp?op=next&currentpage=<%=i_currentpage%>">下一页</a>
<%          }else{%>下一页<%}%>&nbsp;
<%          if(i_currentpage!=i_totalpage){%>
              <a href="huifu.jsp?op=lastpage">末页</a>
<%          }else{%>末页<%}%>
			 </div></td>
        </tr>
        <tr> 
...
<%
  rs=stmt.executeQuery("select * from notess order by id desc  "); 
  if(i_currentpage>i_totalpage)
  i_currentpage=i_totalpage;
  ...
  for(int i=0;i<i_pagenum;i++){
    if(!rs.isLast()){
	  ...
	  session.setAttribute("subject",s_subject);
...
%>		
	  ...
	  <td ><%=s_advice%></td>
<%
  if(s_userid.equals(s_createid)){
%>	
	  <td ><a href="bottom2.jsp">删除</a></td>
	        <%}%>
	</tr>
<%
    }//if
...
<%
  if(s_userid!=null){
%>	
<form name="form" action="huifu2.jsp" method="post">
        <tr> 
          <td width="9%" >&nbsp;</td>
          <td colspan="5">&nbsp;</td>
        </tr>								
...
<%
  rs=stmt.executeQuery("select * from notess where id='"+id+"' "); 
  if(i_currentpage>i_totalpage)
  i_currentpage=i_totalpage;
  int i_position=(i_currentpage-1)*i_pagenum;
  if(i_position==0)
...
<%
  if(s_userid!=null){
%>	
<form name="form" action="huifu3.jsp" method="post">
        <tr> 
          <td width="9%" >&nbsp;</td>
          <td colspan="5">&nbsp;</td>
        </tr>								
